        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title btn btn-success">Import Route Setup</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              <form id="upload_form" action="{{ route('route.importRouteSetup') }}" method="POST" enctype="multipart/form-data">
                @csrf
    ...
    <table id="example"  class="table text-center table-responsive table-striped table-bordered mt-5" style="width:100%;">
            <thead style="font-size: 11px;" class="bg-warning">
                <tr>
                    ...
                    <th class="" width="20"></th>
                    <th class="col">Route Setup Teritory</th>
                    <th class="col">Route Setup Route</th>
                    <th class="col">Route Setup Status</th>
                </tr>
            </thead>
            <tbody style="font-size: 13px;">
                @php
                    $no = 1;
                @endphp
                @foreach ($route_setup as $routeSetup)
                <tr>
                    {{-- <td>{{ $no++ }}</td> --}}
                    <td>
                    ...
                    <td>{{ $routeSetup->route_setup_teritory }}</td>
                    <td>{{ $routeSetup->route_setup_route }}</td>
                    <td>{{ $routeSetup->route_setup_status }}</td>
