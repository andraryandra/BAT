...
@section('title', 'Route Assign Outlet')
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title btn btn-success">Import Route Assign Outlet</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              <form id="upload_form" action="{{ route('route.importRouteAssignOutlet') }}" method="POST" enctype="multipart/form-data">
                @csrf
    ...
    <table id="example"  class="table text-center table-responsive table-striped table-bordered mt-5" style="width:100%;">
            <thead style="font-size: 11px;" class="bg-warning">
                <tr>
                    ...
                    <th class="" width="20"></th>
                    <th class="col">Outlet Order</th>
                    <th class="col">Outlet Customer Number</th>
                    <th class="col">Outlet Customer Name</th>
                    <th class="col">Outlet Address</th>
                </tr>
            </thead>
            <tbody style="font-size: 13px;">
                @php
                    $no = 1;
                @endphp
                @foreach ($route_assign_outlet as $routeAssignOutlet)
                <tr>
                    {{-- <td>{{ $no++ }}</td> --}}
                    <td>
                    ...
                    <td>{{ $routeAssignOutlet->route_assign_outlet_order }}</td>
                    <td>{{ $routeAssignOutlet->route_assign_outlet_customer_number }}</td>
                    <td>{{ $routeAssignOutlet->route_assign_outlet_customer_name }}</td>
                    <td>{{ $routeAssignOutlet->route_assign_outlet_address }}</td>
