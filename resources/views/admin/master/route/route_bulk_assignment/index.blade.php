...
@section('title', 'Route Bulk Assignment')
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title btn btn-success">Import Route Bulk Assignment</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              <form id="upload_form" action="{{ route('route.importRouteBulkAssignment') }}" method="POST" enctype="multipart/form-data">
                @csrf
    ...
    <table id="example"  class="table text-center table-responsive table-striped table-bordered mt-5" style="width:100%;">
            <thead style="font-size: 11px;" class="bg-warning">
                <tr>
                    ...
                    <th class="" width="20"></th>
                    <th class="col">Trade Approach</th>
                    <th class="col">Teritory</th>
                    <th class="col">Route</th>
                    <th class="col">Customer Number</th>
            ...
            <tbody style="font-size: 13px;">
                @php
                    $no = 1;
                @endphp
                @foreach ($route_bulk_assignment as $routeBulkAssignment)
                <tr>
                    {{-- <td>{{ $no++ }}</td> --}}
                    <td>
                    ...
                    <td>{{ $routeBulkAssignment->route_bulk_assignment_trade_approach }}</td>
                    <td>{{ $routeBulkAssignment->route_bulk_assignment_teritory }}</td>
                    <td>{{ $routeBulkAssignment->route_bulk_assignment_route }}</td>
                    <td>{{ $routeBulkAssignment->route_bulk_assignment_customer_number }}</td>
                    <td>{{ $routeBulkAssignment->route_bulk_assignment_customer_name }}</td>
                    <td>{{ $routeBulkAssignment->route_bulk_assignment_order }}</td>
                    <td>{{ $routeBulkAssignment->route_bulk_assignment_address }}</td>
