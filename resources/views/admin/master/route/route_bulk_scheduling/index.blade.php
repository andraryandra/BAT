...
@section('title', 'Route Bulk Scheduling')
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title btn btn-success">Import Route Bulk Scheduling</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              <form id="upload_form" action="{{ route('route.importRouteBulkScheduling') }}" method="POST" enctype="multipart/form-data">
                @csrf
    ...
    <table id="example"  class="table text-center table-responsive table-striped table-bordered mt-5" style="width:100%;">
            ...
            <tbody style="font-size: 13px;">
                @php
                    $no = 1;
                @endphp
                @foreach ($route_bulk_scheduling as $routeBulkScheduling)
                <tr>
                    {{-- <td>{{ $no++ }}</td> --}}
                    <td>
                    ...
                    <td>{{ $routeBulkScheduling->route_bulk_scheduling_teritory }}</td>
                    <td>{{ $routeBulkScheduling->route_bulk_scheduling_trade_approach }}</td>
                    <td>{{ $routeBulkScheduling->route_bulk_scheduling_route }}</td>
                    <td>{{ $routeBulkScheduling->route_bulk_scheduling_route_date }}</td>
